	...
	public class NpgsqlTypesConverter : JsonConverter {
		...
		private object YieldJToken(Type ctype, JToken jt, int rank) {
			...
			if (rank == 0) {
				...
				if (ctype == typeof_NpgsqlRange_int || ctypeGenericType1 == typeof_NpgsqlRange_int) return Executer.ParseNpgsqlRange<int>(jt.ToString());
				if (ctype == typeof_NpgsqlRange_long || ctypeGenericType1 == typeof_NpgsqlRange_long) return Executer.ParseNpgsqlRange<long>(jt.ToString());
				if (ctype == typeof_NpgsqlRange_decimal || ctypeGenericType1 == typeof_NpgsqlRange_decimal) return Executer.ParseNpgsqlRange<decimal>(jt.ToString());
				if (ctype == typeof_NpgsqlRange_DateTime || ctypeGenericType1 == typeof_NpgsqlRange_DateTime) return Executer.ParseNpgsqlRange<DateTime>(jt.ToString());
